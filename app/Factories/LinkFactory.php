...
class LinkFactory
{
    ...
    public static function createLink($long_url, $is_secret = false, $custom_ending = null, $link_ip = '127.0.0.1', $creator = false, $return_object = false, $is_api = false, $campaign = null)
    {
        /**
        ...
        if (!$is_secret && (!isset($custom_ending) || $custom_ending === '') && (LinkHelper::longLinkExists($long_url, $creator, $campaign) !== false)) {
            // if link is not specified as secret, is non-custom, and
            // already exists in Polr, lookup the value and return
            $existing_link = LinkHelper::longLinkExists($long_url, $creator, $campaign);
