...
class AdminPaginationController extends Controller
{
    ...
    public function renderCampaignCell($link)
    {
        if (env('SETTING_ADV_ANALYTICS')) {
            return $link->campaign . $link->campaign_id;
        } else {
    ...
    public function paginateAdminLinks(Request $request)
    {
        ...
        $admin_links = Link::select(['short_url', 'long_url', 'clicks', 'created_at', 'campaign_id', 'creator', 'is_disabled']);
        return Datatables::of($admin_links)
            ...
            ->editColumn('campaign_id', [$this, 'renderCampaignCell'])
